...
export const projects: Project[] = [
  ...
  },
  {
    ...
    serviceType: '웹서비스',
    period: '2025.03 - 2025.04 (5주)',
    team: '1명',
    organization: '개인 프로젝트',
    description:
      '지금까지 진행했던 프로젝트에 대한 정리와 리뷰를 담고 있으며, 기술 데모와 UI/UX 실험 요소들을 모듈화하여 구성한 Playground를 통해 다양한 사용자 경험을 직접 체험해볼 수 있는 포트폴리오 웹사이트입니다.',
    overview: [
      '프론트엔드 개발자로서의 역량을 체계적으로 보여주기 위해 제작한 포트폴리오 웹사이트입니다.',
      '프로젝트 섹션에서는 지금까지 수행한 다양한 프로젝트의 목적, 기술, 역할, 트러블슈팅, 회고 등을 한눈에 볼 수 있도록 정리했습니다.',
      'Playground 섹션은 기능 단위로 모듈화된 기술 실험 공간으로, 애니메이션, 필터링, 데이터 시각화, 게임 등을 직접 체험할 수 있도록 구성되어 있습니다.',
      '정적 설명에 그치지 않고, 사용자가 인터랙션을 통해 개발자의 사고방식과 구현 역량을 직관적으로 이해할 수 있도록 설계했습니다.',
    ],
    tech: ['React.js', 'GA4 (Google Analytics 4)', 'Git', 'styled-components'],
    ...
    isMobile: false,
    features: [
      'React의 컴포넌트 구조를 활용한 페이지 단위 모듈화 및 구조적 UI 설계',
      'Playground 내 인터랙티브 요소들을 기능 단위로 모듈화하여 체험형 콘텐츠 구현',
      'Framer Motion을 활용해 Playground 내부의 다양한 UX 패턴과 시각적 흐름을 설계',
      '프로젝트 섹션에서 슬라이드 형태의 갤러리 구성 및 상세 정보 모달 구현',
      'styled-components를 활용한 반응형 웹 디자인 및 조건부 스타일링',
      'GA4(Google Analytics 4)를 활용한 페이지뷰 및 사용자 행동 이벤트 추적 기능 도입',
      'GitHub Pages + GitHub Actions 기반 정적 웹사이트 자동 배포 파이프라인 구축',
    ],
    techReasons: {
      'React.js':
        '다양한 페이지와 기능을 구조적으로 구성하기 위해, 컴포넌트 기반의 유연한 UI 설계가 가능한 React를 선택했습니다. 기능 단위로 모듈화를 용이하게 할 수 있어 코드 최적화와 유지보수에 유리하다고 판단했습니다.',
      'styled-components':
        '컴포넌트와 스타일을 동일한 파일에서 관리할 수 있어 구조적인 코드 작성이 가능하며, 조건부 스타일링과 반응형 구현에도 유리하여 UI 일관성과 확장성을 확보하기 위해 선택했습니다.',
      'GA4 (Google Analytics 4)':
        '사용자 행동 데이터 분석을 통해 웹사이트 개선 방향을 도출하고자 GA4를 도입했습니다. 또한, 실무에 사용되는 데이터 분석 도구에 대한 학습과 적용 경험을 쌓기 위해 선정했습니다.',
      Git: '버전 관리와 협업을 위한 필수 도구로서, 프로젝트 이력을 체계적으로 관리하고자 사용했습니다. 특히 GitHub Pages를 통한 정적 웹사이트 배포 자동화(CI/CD) 환경을 구축하기 위해 선정했습니다.',
    },
    roles: [],
    troubleshooting: [
      {
        issue: 'GitHub Pages에서 라우팅 문제 발생',
        problem:
          'GitHub Pages가 SSL 기반 정적 호스팅을 사용하고 있어, 각 프로젝트 데모 페이지를 독립적으로 배포할 경우 일반적인 브라우저 라우팅 방식(history mode)을 사용할 수 없었음',
        solution:
          '해당 문제를 해결하기 위해 React Router에서 해시 기반 라우팅(HashRouter)을 적용하여 경로 문제 없이 접근할 수 있도록 설정함',
      },
      {
        issue: 'GA4에서 해시 라우팅 환경의 경로 추적 문제',
        problem:
          'GA4는 기본적으로 URL 경로 기준으로 페이지뷰를 추적하는데, 해시 라우팅 방식에서는 경로 변화를 제대로 인식하지 못해 정확한 추적이 어려웠음',
        solution:
          '페이지 변경 시 직접 `logPageView()` 함수를 호출하여 수동으로 페이지뷰를 전송함으로써, 해시 라우팅 환경에서도 정확한 사용자 행동 데이터를 수집할 수 있도록 개선함',
      },
    ],
    review: [
      '단순히 프로젝트 결과물을 나열하는 것이 아닌, 사용자가 직접 체험하며 개발 역량을 느낄 수 있도록 포트폴리오 구조를 설계했습니다.',
      '특히 Playground 섹션에서는 인터랙션 중심의 UX 실험과 기술 데모를 모듈화해, 사용자 경험에 집중한 다양한 시도들을 구현해볼 수 있었습니다.',
      '해시 라우팅 기반 환경에서 GA4 추적이 제대로 되지 않는 문제나 GitHub Pages의 제약 같은 실제 서비스 배포 이슈들을 해결하면서, 프론트엔드 개발의 전체 흐름(기획-구현-운영)을 경험할 수 있었던 점이 의미 있었습니다.',
      '이번 프로젝트를 통해 단순한 구현을 넘어서 구조 설계, 사용자 관점의 기능 구성, 운영 측면의 문제 해결까지 통합적으로 고려하는 개발자로 한 단계 성장할 수 있었습니다.',
    ],
  },
  {
    id: 'bedding-home',
