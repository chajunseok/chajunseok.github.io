export const projects = [
  ...
  },
  {
    ...
    description: "지금까지 진행했던 프로젝트에 대한 정리와 리뷰를 담고 있으며, 기술 데모와 UI/UX 실험 요소들을 모듈화하여 구성한 Playground를 통해 다양한 사용자 경험을 직접 체험해볼 수 있는 포트폴리오 웹사이트입니다.",
    overview: [
    ...
    techReasons: {
      "React.js": "다양한 페이지와 기능을 구조적으로 구성하기 위해, 컴포넌트 기반의 유연한 UI 설계가 가능한 React를 선택했습니다. 기능 단위로 모듈화를 용이하게 할 수 있어 코드 최적화와 유지보수에 유리하다고 판단했습니다.",
      "styled-components": "컴포넌트와 스타일을 동일한 파일에서 관리할 수 있어 구조적인 코드 작성이 가능하며, 조건부 스타일링과 반응형 구현에도 유리하여 UI 일관성과 확장성을 확보하기 위해 선택했습니다.",
      "GA4 (Google Analytics 4)": "사용자 행동 데이터 분석을 통해 웹사이트 개선 방향을 도출하고자 GA4를 도입했습니다. 또한, 실무에 사용되는 데이터 분석 도구에 대한 학습과 적용 경험을 쌓기 위해 선정했습니다.",
    ...
    troubleshooting: [
      {
        ...
        problem: "GitHub Pages가 SSL 기반 정적 호스팅을 사용하고 있어, 각 프로젝트 데모 페이지를 독립적으로 배포할 경우 일반적인 브라우저 라우팅 방식(history mode)을 사용할 수 없었음",
        solution: "해당 문제를 해결하기 위해 React Router에서 해시 기반 라우팅(HashRouter)을 적용하여 경로 문제 없이 접근할 수 있도록 설정함"
      },
      {
        issue: "GA4에서 해시 라우팅 환경의 경로 추적 문제",
        problem: "GA4는 기본적으로 URL 경로 기준으로 페이지뷰를 추적하는데, 해시 라우팅 방식에서는 경로 변화를 제대로 인식하지 못해 정확한 추적이 어려웠음",
        solution: "페이지 변경 시 직접 `logPageView()` 함수를 호출하여 수동으로 페이지뷰를 전송함으로써, 해시 라우팅 환경에서도 정확한 사용자 행동 데이터를 수집할 수 있도록 개선함"
      }
    ],    
    review: [
  ...
  },
  {
    ...
    techReasons: {
      "React.js": "컴포넌트 기반 구조를 통해 반복되는 UI 요소들을 효율적으로 관리하고, 상태 변화에 따른 실시간 렌더링이 용이하여 쇼핑몰 상품 정보, 마이페이지, 관리자 페이지 등 다양한 페이지를 빠르고 유연하게 구현할 수 있었음.",
